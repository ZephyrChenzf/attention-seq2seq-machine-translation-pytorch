...
encoder.load_state_dict(torch.load('./model/encoder_model.pth'))
decoder.load_state_dict(torch.load('./model/decoder_model.pth'))
...
if use_attention:
    for di in range(y.size(1)):
        ...
        ni = topi[0][0]
        output_words.append(lang_dataset.output_lang.index2word[ni])
    ...
    for di in range(y.size(1)):
        ...
        if ni == EOS_token:
            break
print("model_output: "+' '.join(output_words))